---
layout: default
title: "Lab 06 — Containerization Walkthrough"
parent: Labs
nav_order: 16
---
# Lab 06: Containerization with Docker â€” Visual Walkthrough

**Course:** Software Development Modernization  
**Module:** 06 â€” Containerization with Docker  
...
Labs 01â€“05 brought you from *discovery* through *quality gates*. Lab 06 is the **packaging milestone** â€” the moment where your modernized .NET app stops being a source-code project and becomes a portable, deployable artifact.

This matters for three reasons:

1. **Containers eliminate "works on my machine."** A Docker image encapsulates the exact runtime â€” .NET 8 ASP.NET Core, dependencies, config â€” so the same image runs identically on a developer's laptop, the CI pipeline, and Azure. No more manual server configuration to replicate.
...
| Azure Container Registry | Private registry to store the image | Azure Portal â†’ ACR |
...
| Item | How to verify |
|---|---|
| Docker Desktop installed | `docker --version` â†’ `Docker version 24.x+` |
| .NET SDK 8.x | `dotnet --version` â†’ `8.x.x` |
| Azure CLI logged in | `az account show` â†’ shows iis-student subscription |
| eShopOnWeb repo cloned | Explorer shows `src/Web/Dockerfile` |
| Lab 04 complete | API endpoint builds and passes integration tests |

---

## Part 1 â€” Explore the Existing Dockerfile

### Step 1.1 â€” Open the Dockerfile in eShopOnWeb
...
The existing `src/Web/Dockerfile` is a **two-stage build** â€” the gold standard for containerizing .NET apps. It uses Microsoft's official base images from `mcr.microsoft.com`.
...
| `COPY --from=build` | Copies only the published output â€” not source code or SDK |
| `ENTRYPOINT` | Sets the process that runs when the container starts |

> **App modernization connection:** The multi-stage pattern is critical for security. The production image never contains source code, build tools, or development dependencies â€” only the compiled app. Smaller images also mean faster pull times in CI/CD pipelines.

> âš ï¸ **Snag â€” Build from repo root:** The Dockerfile comments are explicit: build must run from the folder containing the `.sln` file, not from `src/Web/`. Run `docker build -f src/Web/Dockerfile .` from the repo root, not from inside `src/Web/`.

---

## Part 2 â€” Explore the docker-compose Configuration

### Step 2.1 â€” Review docker-compose.yml
...
**What you are looking at:**  
`docker-compose.yml` defines the **multi-container stack** â€” the web app and its SQL Server dependency running as a coordinated set of containers.
...
| `ports` | Maps host port â†’ container port (e.g., `5106:5106`) |
...
> **App modernization connection:** `docker-compose` is the local development equivalent of a Kubernetes deployment manifest. Students who understand compose translate easily to AKS/OpenShift YAML because the concepts â€” services, ports, environment variables, volume mounts â€” are identical. Learn compose â†’ learn Kubernetes faster.

---

## Part 3 â€” Build the Image Locally

### Step 3.1 â€” Microsoft Learn: Containerize a .NET App
...
![Microsoft Learn tutorial: Containerize a .NET application â€” showing docker build command examples and multi-stage Dockerfile guidance](lab-06-screenshots/ss08-ms-learn-docker.png)

**What you are looking at:**  
The official Microsoft Learn tutorial for containerizing .NET apps. This is the reference students should bookmark â€” it covers the full workflow from `docker build` through `docker run` with health checks.
...
# Watch the output â€” you will see:
...
> **App modernization connection:** The `--pull` flag forces Docker to fetch the latest base image. This is important in CI/CD â€” you want to pick up security patches in Microsoft's base images automatically, not pin to a stale layer.

---

## Part 4 â€” Run the Container and Validate

### Step 4.1 â€” Run and Test
...
# In a second terminal â€” test the health endpoint
...
| SQL connection errors | Normal when using in-memory mode â€” set `UseOnlyInMemoryDatabase=true` |
| `CORS policy` warnings | Expected in Development mode â€” not a problem |

> âš ï¸ **Snag â€” SQL connection failure on startup:** If you run without `UseOnlyInMemoryDatabase=true` and no SQL Server is available, the app may fail to start. Add `-e UseOnlyInMemoryDatabase=true` to the `docker run` command for the local validation step. Use docker-compose with the SQL container for full integration testing.

---

## Part 5 â€” Create an Azure Container Registry

### Step 5.1 â€” Open the Azure Portal ACR Creation Form

Navigate to:
```
https://portal.azure.com â†’ Container registries â†’ + Create
```

![Azure Portal â€” Create container registry form showing Subscription (iis-student-az-10), Resource group, Registry name (.azurecr.io), Standard pricing plan](lab-06-screenshots/ss02-create-acr.png)

**What you are looking at:**  
The Azure Container Registry (ACR) creation form. ACR is a private Docker registry hosted in Azure â€” equivalent to Docker Hub but inside your Azure subscription with RBAC access control.
...
## Part 6 â€” Push the Image to ACR

### Step 6.1 â€” Tag and Push
...
## Part 7 â€” Deploy to Azure App Service (Container)

### Step 7.1 â€” Create Web App with Container Publish

Navigate to:
```
https://portal.azure.com â†’ App Services â†’ + Create â†’ Web App
```

![Azure Portal â€” Create Web App form showing Publish: Container option, with Code/Container radio buttons and Linux OS](lab-06-screenshots/ss03-azure-app-service.png)

**What you are looking at:**  
The Azure App Service Web App creation form. The key field is **Publish: Container** â€” switching from "Code" to "Container" changes the deployment model from source-code deployment to image-based deployment from ACR.
...
**After creating â€” configure the container:**
1. In the Web App â†’ **Deployment Center** â†’ **Container settings**
2. Source: `Azure Container Registry`
3. Registry: `eshopmod<yourname>`
4. Image: `eshopweb`
5. Tag: `v1.0`
6. Save â†’ triggers a new deployment

> **App modernization connection:** App Service with container publish is the **Replatform** sweet spot for .NET web apps: no code changes, no Kubernetes cluster to manage, just run your existing image on a fully managed PaaS platform. The app gets auto-scaling, SSL/TLS, custom domains, deployment slots, and monitoring â€” all without infrastructure management.

---

## Part 8 â€” View ADO Pipelines and Course Lab Card

### Step 8.1 â€” ADO Pipelines for Container CI/CD
...
![ADO Pipelines page for Software_Dev_Mod project â€” showing build pipelines list](lab-06-screenshots/ss06-ado-pipelines.png)
...
```yaml
# azure-pipelines.yml â€” Docker build and push stage
...
### Step 8.2 â€” Course Site Lab 06 Card

![Course site Lab 06 card â€” Containerization with Docker, Core MVP Lab tier](lab-06-screenshots/ss07-course-site-lab6.png)
...
| S-04 | App Service shows "Application Error :(" after container deploy | Container is not listening on the expected port | Set `WEBSITES_PORT=5106` in App Service â†’ Configuration â†’ Application settings |
