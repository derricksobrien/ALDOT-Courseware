...
```
Epic            -> represents a major modernization theme
  Feature       -> a deliverable outcome within that theme
    User Story  -> a testable slice of value
      Task      -> concrete implementation step (hours-level)
```
...
## Part 2 — Create the Modernization Epic and Feature Structure

You will create **two epics** that map to the two days of the course.

### Step 2.1 — Create Epic: Day 1 — Foundations

1. In the left sidebar click **Boards → Backlogs**.
...
4. Type the title:
   ```
   Day 1 — Modern Development Foundations and AI-Assisted Coding
   ```
...
### Step 2.2 — Create Epic: Day 2 — Automation and Cloud Delivery

1. Repeat Step 2.1 with the title:
   ```
   Day 2 — Containerization, CI/CD, and Azure Deployment
   ```
2. Fill in:
   - **Description:** Covers Docker, Kubernetes/OpenShift, GitHub Actions pipelines, Azure deployment, and the capstone.
3. Click **Save & Close**.

### Step 2.3 — Add Features Under Day 1
...
### Step 2.4 — Add Features Under Day 2
...
## Part 3 — Create the Sprint

### Step 3.1 — Define Sprint 1

1. In the left sidebar click **Project settings → Boards → Team configuration**.
2. Under **Iterations**, click **+ New child** (under the root path).
3. Name it:
   ```
   Sprint 1 — Course Delivery
   ```
4. Set dates to match the course dates (e.g. Day 1 start through Day 2 end).
5. Click **Save**.

### Step 3.2 — Assign the Sprint to your Team

1. Still in Team configuration, click the **Iterations** tab.
2. Click **Select iterations** and check **Sprint 1 — Course Delivery**.
3. Click **Save**.

---

## Part 4 — Add User Stories and Tasks

You will now create one User Story per lab and add tasks under each. Start with Feature F1 (Work Item Tracking).

### Step 4.1 — Add User Story for Lab 02 (this lab)

1. Go to **Boards → Backlogs** and switch level to **Stories**.
2. Expand **Feature F1 — Work Item Tracking and Sprint Planning**.
...
4. Open the story and fill in:
   - **Acceptance Criteria:**
     ```
     - Work item hierarchy (Epic → Feature → Story → Task) is in place
     ...
     ```
   - **Story Points:** 2
   - **Iteration:** Sprint 1 — Course Delivery
5. Save the story.

### Step 4.2 — Add Tasks to the Lab 02 Story
...
### Step 4.3 — Seed Stories for Remaining Labs
...
|---|---|---|---|
| Lab 03 | F2 — AI-Assisted C# Refactoring | As a developer, I can use GitHub Copilot to refactor legacy C# and generate unit tests. | 3 |
| Lab 04 | F3 — Modern .NET API | As a developer, I can add a SQL Server-backed REST endpoint to eShopOnWeb using minimal APIs. | 3 |
| Lab 05 | F4 — Test Automation | As a QA engineer, I can design and execute a Tosca test suite against the reference app. | 3 |
| Lab 06 | F5 — Containerization | As a developer, I can containerize the eShopOnWeb app and push the image to Azure Container Registry. | 3 |
| Lab 07 | F6 — Kubernetes/OpenShift | As a DevOps engineer, I can deploy the container to OpenShift and expose it via a route. | 3 |
| Lab 08 | F7 — CI/CD Pipeline | As a DevOps engineer, I can create a GitHub Actions workflow that builds, tests, and deploys on every push. | 3 |
| Lab 09 | F8 — Azure Operations | As an operator, I can deploy the app to Azure with Application Insights monitoring enabled. | 3 |

Set all stories to **Sprint 1 — Course Delivery** and **State: New**.

---

## Part 5 — Create Queries and a Dashboard

### Step 5.1 — Create a Sprint Query

1. In the left sidebar click **Boards → Queries**.
2. Click **New query**.
3. Configure the query:
   ...
   | Iteration Path | Under | Software_Dev_Mod\Sprint 1 — Course Delivery |
   | State | <> | Removed |

4. Click **Run query** and verify you see all 8 User Stories.
5. Click **Save query** and name it:
   ```
   Sprint 1 — All User Stories
   ```
6. Save it under **My Queries** (or **Shared Queries** if the instructor grants access).

### Step 5.2 — Add Widgets to the Dashboard

1. Go to **Overview → Dashboards → Software_Dev_Mod Team - Overview**.
...
3. Add the following widgets (click **+ Add widget** for each):
   ...
   | **Work Item Count** | Query: Sprint 1 — All User Stories |
...
## Part 6 — Link Work Items to Source Control

### Step 6.1 — Connect Your GitHub Repo
...
### Step 6.2 — Link a Commit to a Work Item
...
| # | Check | Done |
|---|---|---|
| 1 | Two Epics exist with correct titles | ☐ |
| 2 | Eight Features are created under the correct Epics | ☐ |
| 3 | Eight User Stories exist with Acceptance Criteria, Story Points, and Sprint assigned | ☐ |
| 4 | Lab 02 story has 5 tasks with hours estimates | ☐ |
| 5 | Sprint 1 is defined with start and end dates | ☐ |
| 6 | Query "Sprint 1 — All User Stories" returns all 8 stories | ☐ |
| 7 | Dashboard shows Sprint Burndown, Work Item Count, and Velocity widgets | ☐ |
| 8 | At least one commit is linked to a work item (or GitHub connection confirmed) | ☐ |

---

## Evidence to Submit

1. **Screenshot:** ADO backlog showing Epic → Feature → Story hierarchy
...
- If sprint dates need to be adjusted, do so in **Project Settings → Boards → Team configuration → Iterations** before the class starts.
- The GitHub connection in Step 6.1 requires the instructor to have GitHub org admin rights. Pre-configure this if possible.
- The Tosca User Story (Lab 05) may be removed from the sprint if Tosca licenses are not available — replace with a placeholder Bug titled "Tosca environment not provisioned."
