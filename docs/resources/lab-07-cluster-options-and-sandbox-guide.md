...
# Lab 07 Cluster Options â€” OpenShift Sandbox, Docker Desktop, and Azure
...
|---|---|---|---|
| Cost | Shared org cluster (free) or Red Hat Developer Sandbox (free, 30-day) | Free on the student machine | Pay-per-use â€” requires Azure subscription |
| Setup complexity | Low if pre-provisioned; medium if students provision their own | Very low â€” enable in Docker Desktop settings | Medium â€” AKS requires CLI, resource group, and cluster creation |
| Realism to enterprise | Highest â€” matches what many government and regulated orgs run | Low â€” local only, no real cluster concepts | High â€” widely used in enterprise, different from OpenShift |
| OpenShift-specific features | Full â€” Routes, SCC, oc CLI, built-in registry | None â€” plain Kubernetes only | None â€” AKS uses standard Kubernetes |
| Works offline | No â€” requires network access | Yes â€” runs completely locally | No â€” requires internet and Azure |
| Student machine requirements | Light â€” just oc or kubectl CLI | Heavy â€” 8â€“16 GB RAM minimum | Light â€” just Azure CLI |
| Persistent across class days | Yes if shared cluster; ephemeral for sandbox | Yes â€” local state persists | Yes â€” resources persist until deleted |
...
| Registry integration | Built-in OpenShift internal registry | Uses local Docker daemon | ACR â€” images must be pushed there first |
...
Do students have access to an OpenShift cluster or Red Hat Developer Sandbox?
  YES â†’ Use OpenShift. Highest realism, covers oc, Routes, SCC.
  NO  â†’ Do student machines have 16 GB RAM and Docker Desktop installed?
          YES â†’ Use Docker Desktop Kubernetes. Zero cost, works offline.
          NO  â†’ Does the class already have Azure subscriptions active?
                  YES â†’ Use AKS or ACA. Ties naturally into Lab 09.
                  NO  â†’ Use Red Hat Developer Sandbox (free 30-day account).
```

---

## Red Hat Developer Sandbox â€” Full Guide

### What it is

The Red Hat Developer Sandbox is a free, 30-day, shared OpenShift cluster that any developer can access with a Red Hat account. It gives each student a personal namespace on a real OpenShift cluster hosted by Red Hat â€” no infrastructure setup, no credit card required.
...
1. Click the username in the top-right corner â†’ **Copy login command**
2. Click **Display token**
3. Copy the `oc login` command â€” it looks like:
...
For the dotnet S2I path (build from source directly in the cluster â€” **live-tested 2026-08-10, working**):
...
- Build: âœ… Complete (55s â€” .NET 8 S2I build)
- Pod: âœ… Running (`1/1`)
- Route: `dotnet-ex-derricksobrien-dev.apps.rm2.thpm.p1.openshiftapps.com`
- HTTP 200: âœ… App serving ASP.NET Core MVC home page
- ConfigMap: âœ… Created
- HPA: âœ… Created (1â€“3 replicas, 70% CPU target)

### Constraints and impact on Lab 07

| Constraint | Impact on Lab 07 |
|---|---|
| No cluster-admin | Cannot create ClusterRoles or modify SCCs â€” no impact on core lab steps |
| Single namespace | All work stays in one namespace â€” no issue for this lab |
...
| Image pull from external registries | Works â€” may need imagePullSecrets for private registries |

### What works fine for Lab 07

- Routes work fully â€” students get real HTTPS URLs immediately
...
- [ ] **Sign students up for Red Hat accounts at least 24 hours before Lab 07** â€” new accounts require manual approval by Red Hat administrators
...
2. Activate the sandbox at https://sandbox.redhat.com â†’ click **Try it** next to OpenShift
3. Wait for approval email (new accounts may require manual review â€” typical wait: a few hours)
...
1. **Shared org OpenShift cluster** â€” if your organization has one provisioned for the course
2. **Red Hat Developer Sandbox** â€” best zero-cost fallback, real OpenShift experience, HTTPS routes included
3. **Docker Desktop Kubernetes** â€” for offline or resource-constrained environments
4. **AKS or ACA** â€” if the class already has Azure subscriptions active for Lab 09
